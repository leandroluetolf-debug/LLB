 ...
 * event tags. We never read or display Oura's encrypted readiness/sleep scores. LLB computes its
 * own Charge/Rest downstream:
 *   - the IBI stream becomes [Streams.rr], from which RecoveryScorer reconstructs LLB's OWN RMSSD;
 ...
 *     scale is not Tier-A; LLB's scoring RMSSD comes from `rr`, not this tag);
        ...
        for (ev in events) {
            when (ev) {
                ...
                is OuraEvent.Hrv -> {
                    ...
                    // readiness score, and NOT used as LLB's RMSSD (that comes from `rr`).
                    val ts = anchor(ev.value.ringTimestamp) ?: continue
                    out.events.add(
                        WhoopEvent(
                            ts = ts,
