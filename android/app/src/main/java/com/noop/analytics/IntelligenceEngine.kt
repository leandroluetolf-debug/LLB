 ...
 *     covered.
 *
 * This is what makes LLB independent of WHOOP's cloud , for any day the strap
 * collected raw data with LLB connected, LLB scores it itself rather than relying on
...
object IntelligenceEngine {
    ...
    /** Imported wearable-export source ids whose DAILY aggregates can be scored for a LLB Charge/Rest on
     ...
     * moved from the old 0–21 axis to LLB's own 0–100 axis. On-device computed rows since v2.6.0 already
    ...
    private suspend fun analyzeRecentOnCpu(
        ...
        for (offset in 0 until maxDays) {
            ...
            // over the whole asleep span. The mean always sits above the floor, so LLB looking lower is
        ...
        // untouched; the computed row carries the LLB-derived Charge + the Rest composite. HONEST DATA: the
     ...
     * over the whole asleep span. The mean always sits at-or-above the floor, so LLB reading lower is
     * BY DESIGN, not a bug; logging both makes a "LLB RHR is lower than my other app" report explainable
