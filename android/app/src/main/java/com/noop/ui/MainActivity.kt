...
object NoopPrefs {
    ...
    /** Whether LLB should hold the strap connection open via a foreground service. Default true. */
    ...
    /** Whether LLB keeps the dense realtime HR stream armed 24/7 for continuous HRV capture. Default
    ...
    /** Whether LLB re-broadcasts its live HR as a standard BLE Heart Rate peripheral. Default OFF. */
    ...
    /** Health Connect periodic auto-sync (Samsung Health → Health Connect → LLB). Default OFF.
    ...
    /** Health Connect writeback (LLB's computed metrics → HC, for other apps). Default OFF. */
    ...
    /** "Auto-detect workouts" (MVP, opt-in, on-device, NON-DESTRUCTIVE). When ON, LLB scans the last
    ...
    /** The last strap we bonded to (address + model), persisted so LLB can reconnect to it directly on
