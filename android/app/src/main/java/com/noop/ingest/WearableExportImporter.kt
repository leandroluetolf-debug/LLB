 ...
 *              REAL Oura export schema (issue #862). The many health types LLB doesn't model
 ...
 *
 * Maps onto LLB's DAILY metrics + sleep sessions (NOT workouts). HONEST DATA: only fields the export
 * carries are written; a brand's OWN score (Oura "readiness", a sleep score) is stored under a
 * reference metricSeries key only — NEVER as LLB's Charge/Effort/Rest. Every row is written under the
 ...
 *
 * LICENSE: the file shapes are DOCUMENTED format facts. No GPL/AGPL code is copied — LLB's own code.
 */
object WearableExportImporter {
    ...
    private suspend fun persist(repo: WhoopRepository, brand: Brand, parsed: Parsed): ImportSummary {
        ...
        for (d in parsed.days) {
            ...
            // Reference-only: the brand's own scores, never a LLB Charge/Effort/Rest input.
