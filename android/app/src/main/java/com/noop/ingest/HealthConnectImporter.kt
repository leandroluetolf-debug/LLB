...
object HealthConnectImporter {
    ...
    suspend fun import(context: Context, repo: WhoopRepository, heightCm: Double = 0.0): ImportSummary {
        ...
        // granted. The user choosing exactly what LLB can see is the intended behaviour.
        if (granted.none { it in PERMISSIONS }) {
            return ImportSummary.failure(
                SOURCE,
                "No Health Connect data types are granted. Allow at least one type for LLB in Health Connect, then import.",
            )
        }
        ...
        for ((day, a) in acc) {
            ...
            if (hasApple) {
                ...
                // so DERIVE it from the day's weight + the user's PROFILE height — the same height LLB
        ...
        try {
            do {
                ...
                for (record in response.records) {
                    // #528: never re-ingest what LLB itself wrote to Health Connect, or "share back"
    ...
    /**
     * #528 — true when a record's origin is LLB itself, so [readAll] skips it on import. Without this,
     * turning on "share back" makes a later import re-read LLB's own daily totals and SUM them on top
