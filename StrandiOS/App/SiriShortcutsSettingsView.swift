...
/// Surfaces LLB's already-registered App Intents (see StrandiOS/System/NOOPAppIntents.swift) in the
/// UI so users discover them. `NOOPShortcuts` auto-registers "Buzz Band" and "Mark a Moment" with
...
struct SiriShortcutsSettingsView: View {
    var body: some View {
        ScreenScaffold(title: "Siri & Shortcuts",
                       subtitle: "Run LLB actions hands-free.") {
            tips
            shortcutsCard
        }
    }
    ...
    private var shortcutsCard: some View {
        StrandCard(padding: 20) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("Wire LLB's actions into a Back-Tap, a focus automation, or a longer Shortcut. For example, double-tap the back of your iPhone to buzz the strap.")
                    .font(StrandFont.caption)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
                ShortcutsLink()
