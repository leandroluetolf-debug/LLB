...
/// respiration ~5%; strain 0–21 from cardiovascular load). This is what makes LLB independent of
/// WHOOP's cloud , for any day the strap collected raw data with LLB connected, LLB scores it
...
final class IntelligenceEngine: ObservableObject {
    ...
    /// Who supplies the dashboard headline for a By-Day row. The By-Day card always shows LLB's OWN
    ...
    /// imported one, instead of always claiming "LLB-computed". (Sleep overhaul §2.6 honesty fix.)
    enum DaySource: Equatable {
        /// LLB scored this day itself from the raw strap streams; no import covers it.
        case computed
    ...
    /// The per-day RHR floor-vs-mean diagnostic line (#691). LLB's `floor` is the WHOOP-style resting
    ...
    /// over the whole asleep span. The mean always sits at-or-above the floor, so LLB reading lower is BY
    /// DESIGN, not a bug; logging both makes a "LLB RHR is lower than my other app" report explainable
    ...
    nonisolated static func rhrFloorMeanLogLine(day: String, floor: Int, inBedBpms: [Int]) -> String {
        ...
        return "rhr day=\(day) floor=\(floor) nightMean=\(meanLog) inBedSamples=\(inBedBpms.count) "
            + "(floor = WHOOP-style lowest-sustained = LLB RHR; mean = sleeping-HR-app number)"
    }
    ...
    /// moved from the old 0–21 axis to LLB's own 0–100 axis. On-device computed rows since v2.6.1
    ...
    func analyzeRecent(maxDays: Int = 21, force: Bool = true) async {
        ...
        let scanned: [DayScan] = await Task.detached(priority: .utility) {
            ...
            for offset in 0..<maxDays {
                ...
                // Make the recurring "LLB's resting HR reads LOWER than my sleeping-HR app" reports
                // explainable from the strap log instead of a guess. The two numbers measure different
                // things BY DESIGN, not a bug: LLB's `restingHr` is the WHOOP-style FLOOR (the lowest
                ...
                // MEAN over the whole asleep span. The mean always sits above the floor, so LLB looking
        ...
        // the computed row carries the LLB-derived Charge + the Rest composite. HONEST DATA: the engine
        ...
        results = out
        note = out.isEmpty
            ? "No scored nights yet. Wear the strap with LLB connected overnight and the engine will score your charge, effort and rest itself, no WHOOP cloud required."
            : nil
