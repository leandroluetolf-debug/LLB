...
/// under the ring's `deviceId`, and lets LLB compute its own Charge/Rest from those streams exactly like
...
public final class OuraLiveSource: NSObject, ObservableObject {
    ...
    /// Set to an HONEST explanation string when the ring needs a pairing/key handshake LLB can't complete
    ...
    /// Record + log the honest "this ring needs a pairing handshake LLB can't complete" outcome (once),
    ...
    private func announceNeedsPairing(reason: NeedsPairingReason) {
        ...
        switch reason {
        case .factoryResetOrNoKey:
            detail = "LLB needs the ring's install key to read it live, and that pairing handshake isn't set up yet."
        case .authFailed(let status):
            detail = "The ring rejected the pairing handshake (status \(status.rawValue))."
        case .installFailed(let why):
            detail = "LLB couldn't take over this ring (\(why))."
        }
