...
//
// A full-screen, paged onboarding + pairing flow for LLB. Cinematic and calm:
...
//
// Steps:
//  1 Welcome           — LLB + "all your data, none of the cloud"
...
private struct WhatItDoesStep: View {
    ...
    var body: some View {
        StepShell(title: String(localized: "What LLB does"), subtitle: String(localized: "Three quiet promises.")) {
            VStack(spacing: 14) {
        ...
        StepShell(title: String(localized: "What to expect"),
            ...
            VStack(spacing: 12) {
                ...
                expectationRow(
                    ...
                    body: String(localized: "On iPhone this is a sideloaded build. Re-sign it about every 7 days on a free Apple ID (longer on a paid account). After your phone reboots, unlock it once so LLB can read and sync its data.")
                )
                .opacity(shown ? 1 : 0)
                .offset(y: shown ? 0 : 8)
        ...
        StepShell(title: String(localized: "A quick word before we connect"),
            ...
            VStack(spacing: 24) {
                ...
                InfoCard(
                    ...
                    message: String(localized: "LLB talks to your strap directly over Bluetooth Low Energy. There's no server in the middle. The connection is local, and so is every reading it pulls in.")
                )

                Text("When the system prompt appears, choose Allow so LLB can find your strap.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .multilineTextAlignment(.center)
                    .frame(maxWidth: 460)
...
private struct ScanStep: View {
    ...
    var body: some View {
        StepShell(title: String(localized: "Find your strap"),
                  subtitle: live.bonded ? String(localized: "Bonded. You're set.") : String(localized: "Pick your strap below, then tap Scan. LLB will find it.")) {
            VStack(spacing: 24) {
    ...
    private var reassurance: some View {
        StrandCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("WHOOP straps don't appear in your \(Platform.deviceNoun)'s Bluetooth settings. They advertise on a custom profile that only apps like LLB can find, so there's nothing to pair there, and you shouldn't try.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
...
private struct NotificationsStep: View {
    ...
    var body: some View {
        StepShell(title: String(localized: "Stay in the loop"),
                  subtitle: String(localized: "LLB can tap your wrist when your \(Platform.deviceNoun) needs you. No glance at the screen required.")) {
            VStack(spacing: 24) {
                ZStack {
                    Circle()
                        .stroke(StrandPalette.accent.opacity(0.25), lineWidth: 2)
                ...
                // keep only what iOS genuinely does: LLB's own strain nudges + smart alarm buzz the strap
                // directly over BLE.
                InfoCard(
                    icon: "applewatch.radiowaves.left.and.right",
                    tint: StrandPalette.statusPositive,
                    title: String(localized: "A buzz, not a banner"),
                    message: String(localized: "LLB taps your strap so an alert lands on your wrist instead of your screen. No need to reach for it. Everything stays on \(Platform.deviceNounPhrase).")
                )
                ...
                InfoCard(
                    ...
                    tint: StrandPalette.statusPositive,
                    title: String(localized: "A buzz, not a banner"),
                    message: String(localized: "When the \(Platform.deviceNoun) apps you choose send a notification, LLB taps your strap: Slack, Calendar, Messages, whatever matters. Everything stays on \(Platform.deviceNounPhrase).")
                )
