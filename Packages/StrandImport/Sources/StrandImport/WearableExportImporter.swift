...
// own account — fully offline, no cloud API, no login. LLB ingests the file the user already owns and
// maps it onto LLB's DAILY metrics + sleep sessions (NOT workouts — workouts stay wave-1's lane).
//
...
// surfaced as LLB's Charge/Effort/Rest. LLB recomputes its own scores downstream from the raw
...
// GDPR wellness). No GPL/AGPL code is copied — this is LLB's own clean implementation.
...
public struct WearableExportImporter {
    ...
    public func `import`(from url: URL) throws -> WearableImportResult {
        ...
        if days.isEmpty && sleeps.isEmpty {
            ...
            // recovery/sleep/HRV LLB can map. Say so plainly and point at the right file (#857) instead of
